...
Since the P-value of the above results is lower than 0.05 and the Z-value is positive, we can use statistical evidence to conclude that the attributes are not randomly distributed, and the spatial distribution of high and/or low values in the data set is more of a spatial clustering property
...
>
> From the figure, we can see that the actual Moran's I statistics (blue line) far exceed the simulated data (blue shading), indicating significant evidence of positive autocorrelation.
...
Question: What statistical conclusion can you draw from the output above?
According to the results, the p value is lower than the alpha value of 0.05, so we can reject the null hypothesis with statistical evidence.
...
based on the plot, we can conclude there is significant evidence of positive autocorrelation.
...
A positive value of I indicates that the feature has similar values of adjacent attributes. This feature is part of the cluster. A negative value of I indicates that a feature has a feature whose adjacent values are not similar; This feature is an outlier. chorpleth shows evidence of positive plus negative Ii values. However, as mentioned above, it is useful to consider the p-value of each value.
